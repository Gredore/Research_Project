...
for train, test in kfold.split(X_train_RDF, y_train_RDF):
    ...
    model.add(Conv1D(512, kernel_size=3, activation='relu', input_shape=([X_train_RDF.shape[1], 1])))
    ...
    history = model.fit(X_train_RDF[train], y_train_RDF[train],validation_data=(X_train_RDF[test], y_train_RDF[test]),class_weight=class_weights_dict, epochs=epochs, verbose=1, callbacks=[es])
